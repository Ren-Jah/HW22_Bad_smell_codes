...
class Electroscooter(Transport):
    ...
    def move(self):
        print("Электроскутер начинает движение.")

    def stop(self):
        print("Электроскутер остановлен.")


class Person:
    def use_transport(self, transport_type):
        transport_type.start_engine()
        transport_type.move()
        transport_type.stop()
        transport_type.stop_engine()
